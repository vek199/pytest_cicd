...
from my_math import (
    ...
    # Exceptions
    MathError, DivisionByZeroError, NegativeNumberError,
    InvalidInputError, EmptySequenceError,
    # Decorators
    timing_decorator, validate_numeric_args, retry,
    # Async functions
    async_add, async_factorial, async_batch_add,
)
...
class TestExceptions:
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("func,args,expected_exception", [
...
class TestMarkers:
    ...
    @pytest.mark.skipif(
        not hasattr(math, 'prod'),
        reason="math.prod not available in Python < 3.8"
    )
...
class TestPropertyBased:
    ...
    @pytest.mark.parametrize("a,b", [
        (i, j) for i in range(-10, 11, 5) for j in range(-10, 11, 5)
    ])
